...
/* Keep the browser preview on the same publishing-avatar catalog as the
 * authoritative backend. The browser never gets to choose the webhook avatar;
 * this only makes the Studio preview faithfully mirror production. */
const STUDIO_IDENTITY_LOGO_FILES = Object.freeze({
...
function studioIdentityLogoUrl(id) {
  const filename = STUDIO_IDENTITY_LOGO_FILES[String(id || "")];
  return filename
    ? `https://commons.wikimedia.org/wiki/Special:Redirect/file/${encodeURIComponent(filename)}?width=512`
    : "";
}

const studioNormalizeIdentityBeforeLogos = normalizeIdentityForClient;
normalizeIdentityForClient = function normalizeIdentityForClientWithLogo(raw) {
  const identity = studioNormalizeIdentityBeforeLogos(raw);
  if (!identity) return null;
  const mapped = studioIdentityLogoUrl(identity.id);
  if (mapped) identity.avatarUrl = mapped;
  return identity;
};
